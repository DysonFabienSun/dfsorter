...
- Video is black. Use a **7 px timeline groove** with a larger hit area, pale teal remainder and strong teal elapsed section/playhead. Use focus-cyan saved I/O markers with bold I/O labels and accent range tint at 18% opacity. Pending In and Out have distinct bold labels (·I and ·O). The volume slider uses an 18 px widget height, 3 px groove, 10 px handle, quiet teal remainder and medium-teal level; center it optically with the volume icon and time text. Transport/audio/time controls remain directly below.
...
```text
Home  Browse  Session  Editing  Export  Config   [stretch]   Undo Redo   Theme Settings
                                                                          Projects tab
------------------------------------------------------------------------------------------------------
Left pane                  | Center: page/player and information                  | Projects
```

Navigation touches application top edge; workspace has a 12 px top, right and bottom inset and a 13 px left inset. This left gutter uses the normal workspace surface while the sidebar keeps a compact 8 px content inset. Use the continuous strip and shared tab styling described above. Undo/Redo, Theme and Settings align on one vertical centerline at a consistent 28 px height and use frameless ghost styling. Undo/Redo have 4 px separation and retain the shared optical 1 px downward offset used by icon-only navigation utilities. The closed Projects control is a 30 px folder-like tab beneath the navigation strip, flush with the right window edge, with no top border, a square lower-right corner and an emphasized rounded lower-left transition into the workspace. A 1 px vertical edge decorator sits 4 px inside its top, bottom and right edges; a compact pixel-aligned 1 px left-pointing chevron in a subtle intermediate border/text tone sits immediately to its left. Neither decorator shifts the folder icon or label. The tab disappears while the pane is open. Every icon-only utility has a tooltip and accessible name.
