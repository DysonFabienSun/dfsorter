...
Navigation touches application top edge; workspace has a 12 px top, right and bottom inset and a 13 px left inset. This left gutter uses the normal workspace surface while the sidebar keeps a compact 8 px content inset. Use the continuous strip and shared tab styling described above. Utilities align on one vertical centerline at a consistent 28 px height and optically consistent icon size; Projects remains a compact 28 px labeled control using the 13 px medium semantic font with vertically centered content. Undo/Redo, Theme and Settings are frameless ghost controls; Projects retains its quiet neutral frame. Undo/Redo have 4 px separation, then 16 px before Projects. Theme sits between Projects and Settings with 4–8 px toolbar spacing. Retain the shared optical 1 px downward offset for icon-only navigation utility artwork. Every icon-only utility has a tooltip and accessible name. Do not imitate tabs with ordinary form buttons.
