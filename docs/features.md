...
## Semantic light/dark visual facelift

- [x] User approved the canonical facelift specification before implementation. The UI Layout Guide now defines semantic Light and Dark palettes, System resolution, component hierarchy, page composition, interaction states and practical contrast targets; the main specification owns persistence and quick-toggle behavior.
- [x] Light is the unsaved default. Settings → Appearance provides System / Light / Dark, and the top-right sun/moon action switches between explicit modes. Selection persists in `data/settings.yaml`; System follows live Qt operating-system color-scheme changes.
- [x] Shared QSS, palettes, rich text, custom-painted clip cards/timelines/ratings and on-demand Lucide icons resolve through one mutable semantic token map. Theme changes clear tinted icon caches and repaint existing UI without application restart or per-component dark-mode branches.
- [x] Neutral hover states, filled primary actions, quiet borders, bottom-indicator navigation and soft clip selection reduce pervasive outlines and accent use. Filter groups now inherit the sidebar surface; Session uses one deliberate compact setup group; Home folder information has clearer line grouping; Export and Config have page hierarchy.
- [x] Follow-up refinement separates the cool canvas, tinted sidebars/top chrome, white workspace and secondary surfaces; unselected clips are flat separator rows; neutral buttons use a shared tinted fill and quieter border; Editing shortcut guidance is a smaller, more separated helper tier. The light accent is a richer, higher-contrast teal while dark mode retains the same semantic hierarchy.
- [x] The native libmpv surface now follows each source's display-corrected aspect ratio. Player-added letterbox/pillarbox regions use the application canvas instead of black, without cropping, stretching or attempting to remove black pixels encoded in the source.
- [x] Focused aspect-ratio checks cover libmpv display dimensions, landscape and portrait geometry, a real decoded H.264 clip, Browse layout and fullscreen surface stability (7 passing cases). A real playback assertion confirms the native surface matches the decoded 320×180 source ratio.
